...
<svelte:head>
  ...
  <meta
    name="description"
    content="Daftar token halal yang sudah di-screening oleh tim CryptoSharia"
  />
  <meta
...
>
  {#if !data.tokens || data.tokens.length === 0}
    <span class="pt-8 text-center text-xl text-slate-500">Token halal tidak ditemukan</span>
  {:else}
